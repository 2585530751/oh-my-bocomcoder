...
- **Provider timeout**: `providers.webSearchTimeoutSeconds` supplies the hard ceiling for each provider's search transport before the automatic chain advances. It defaults to `60`; invalid non-positive values fall back to that default, while provider-specific upstream or aggregate limits may still be shorter.
- **Provider adapters**
