...
describe("initTelemetryExport signals export path", () => {
	it("registers a provider and exports spans to an OTLP/proto receiver", async () => {
		...
		// Ignored stdio sidesteps Bun's intermittent pipe EBADF in test workers;
		// the probe exits 0 only after it observes the export land.
		const proc = Bun.spawn([process.execPath, probe], { stdin: "ignore", stdout: "ignore", stderr: "ignore" });
